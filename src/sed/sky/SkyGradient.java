...
public class SkyGradient {
     ...
     */
    public void updateSunPosition(double timeOfDay, int julianDay, double latitude,
            int standardMeridian, double longitude) {
    ...
    private void sunPosition(double timeOfDay, int julianDay, double latitude,
            int standardMeridian, double longitude) {
        // solarTime in Stunden
        double solarTime = timeOfDay
                + (0.170 * Math.sin(4d * Math.PI * (julianDay - 80d) / 373d) - 0.129 * Math.sin(2d
                        * Math.PI * (julianDay - 8d) / 355d)) + (standardMeridian - longitude)
                / 15d; // Zeitdifferenz zwischen 2 Meridianen betraegt 4 min, 15 Meridiane = 60 min = eine Stunde
        ...
        double solarAltitude = Math.asin(Math.sin(latitude) * Math.sin(solarDeclination)
                - Math.cos(latitude) * temp); // height angle
